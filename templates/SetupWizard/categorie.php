<?php
/**
 * Setup Wizard - Step 2: Categorie Prodotti
 *
 * @var \App\View\AppView $this
 * @var array $categorie
 * @var int $currentStep
 * @var array $steps
 */

$this->assign('title', 'Configurazione - Categorie');
?>

<div class="setup-wizard">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <?= $this->element('SetupWizard/progress', ['currentStep' => $currentStep, 'steps' => $steps]) ?>

            <div class="card border-0 shadow-sm">
                <div class="card-header bg-transparent border-0 pt-4">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            <div class="rounded-circle bg-primary bg-opacity-10 p-3 me-3">
                                <i data-lucide="folder" class="text-primary" style="width:24px;height:24px;"></i>
                            </div>
                            <div>
                                <h4 class="mb-0">Categorie Prodotti</h4>
                                <p class="text-secondary mb-0">Organizza i tuoi prodotti in categorie</p>
                            </div>
                        </div>
                        <?php if (empty($categorie)): ?>
                            <?= $this->Form->create(null, ['url' => ['action' => 'categorie']]) ?>
                            <?= $this->Form->hidden('crea_esempi', ['value' => '1']) ?>
                            <button type="submit" class="btn btn-outline-primary">
                                <i data-lucide="sparkles" style="width:16px;height:16px;" class="me-1"></i>
                                Crea Esempi
                            </button>
                            <?= $this->Form->end() ?>
                        <?php endif; ?>
                    </div>
                </div>

                <div class="card-body">
                    <!-- Existing Categories -->
                    <?php if (!empty($categorie)): ?>
                        <div class="mb-4">
                            <h6 class="text-secondary mb-3">Categorie create:</h6>
                            <div class="row g-2">
                                <?php foreach ($categorie as $cat): ?>
                                    <div class="col-md-6 col-lg-3">
                                        <div class="card bg-body-secondary border-0">
                                            <div class="card-body py-2 px-3">
                                                <div class="d-flex align-items-center">
                                                    <i data-lucide="check-circle" class="text-success me-2" style="width:16px;height:16px;"></i>
                                                    <span class="fw-medium"><?= h($cat->nome) ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    <?php endif; ?>

                    <!-- Add New Category -->
                    <div class="card bg-body-tertiary border-0 mb-4">
                        <div class="card-body">
                            <h6 class="mb-3">
                                <i data-lucide="plus" style="width:16px;height:16px;" class="me-1"></i>
                                Aggiungi Categoria
                            </h6>
                            <?= $this->Form->create(null, ['url' => ['action' => 'categorie']]) ?>
                            <div class="row g-3">
                                <div class="col-md-5">
                                    <?= $this->Form->control('nome', [
                                        'label' => false,
                                        'class' => 'form-control',
                                        'placeholder' => 'Nome categoria',
                                        'required' => false,
                                    ]) ?>
                                </div>
                                <div class="col-md-5">
                                    <?= $this->Form->control('descrizione', [
                                        'label' => false,
                                        'class' => 'form-control',
                                        'placeholder' => 'Descrizione (opzionale)',
                                    ]) ?>
                                </div>
                                <div class="col-md-2">
                                    <button type="submit" class="btn btn-primary w-100">
                                        <i data-lucide="plus" style="width:16px;height:16px;"></i>
                                        Aggiungi
                                    </button>
                                </div>
                            </div>
                            <?= $this->Form->end() ?>
                        </div>
                    </div>

                    <!-- Actions -->
                    <div class="d-flex justify-content-between pt-3 border-top">
                        <a href="<?= $this->Url->build(['action' => 'azienda']) ?>" class="btn btn-outline-secondary">
                            <i data-lucide="arrow-left" style="width:16px;height:16px;" class="me-1"></i>
                            Indietro
                        </a>
                        <?= $this->Form->create(null, ['url' => ['action' => 'categorie']]) ?>
                        <?= $this->Form->hidden('next_step', ['value' => '1']) ?>
                        <button type="submit" class="btn btn-primary">
                            Continua
                            <i data-lucide="arrow-right" style="width:16px;height:16px;" class="ms-1"></i>
                        </button>
                        <?= $this->Form->end() ?>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
